...
class GadgetSearch(object):
    ...
    def get_format_count(self, gadget_format: str) -> int:
        ...
        # credit: https://stackoverflow.com/questions/25996937/how-can-i-extract-keywords-from-a-python-format-string/25997051#25997051
        
        from string import Formatter
        return(len(set([fname for _, fname, _, _ in Formatter().parse(gadget_format) if fname])))
    ...
    def get_register_combos(self, nregs: int, registers: Tuple[str]) -> Iterable[Iterable[str]]:
        ...
        if (nregs == 0):
            ...
            return [combination + [register] for combination in self.get_register_combos(nregs-1, registers) for register in registers]
    ...
    def format_all_gadgets(self, gadget_format: str, registers: Tuple[str]) -> Iterable[str]:
        ...
        combinations = self.get_register_combos(num_of_placeholders, registers)
        for combination in combinations:
            formated_gadgets += [gadget_format.format(*combination)]
        return formated_gadgets
    ...
    def find_all(self, gadget: str) -> Iterable[int]:
        ...
        offsets = []
        start_address = self.start_addr
        memory_dump = self.dump
        gadget_bytes = assemble.assemble_data(gadget + '; RET')

        for offset in range(len(memory_dump)):
            if (memory_dump[offset:offset + len(gadget_bytes)] == gadget_bytes):
                offsets.append(offset)
        
        return ([(offset + int(start_address)) for offset in offsets])
    ...
    def find_all_formats(self, gadget_format: str,
        ...
        for gadget in gadgets:
            ...
            for address in addresses:
                all_formats.append((gadget, address))
